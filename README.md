...
结构与 **`extension/奇臣传`**、**`extension/群友设计`** 对齐，便于只改数据与技能、不动入口装配逻辑。
...
| `src/card/patchAssets.js` | 卡牌插画 `ext:群芳/image/card/...` |
...
1. `src/character/data.js` 增加 id 与 `skills` 数组（建议 id 前缀 `qunfang_`）
...
6. 在 `src/package.js` 的 `characterSort` 中归入扩展分组（可选）
